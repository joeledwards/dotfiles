...
# Retrieve version description as a string, if in git repo
__gitdescribe () {
	local b="$(git describe --always 2>/dev/null)"
	if [ -n "${b}" ]
	then
		if [ -z "$1" ]
		then
...
# Retrieve the project name if in a git repo
__gitproject () {
	local b=$(__gitdir)
	if [ -n "${b}" ]
	then
